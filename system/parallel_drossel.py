...
def simuliere_parallel_drossel(hydraulik, drossel_vorgabe_mm, m_flaeche, m_test_flow,
    ...
    # --- OUTER LOOP: Berechnet die Strömungsaufteilung und den Gegendruck sauber neu ---
    for outer_it in range(15):
        # WICHTIG: Die Grenzen der Suchfunktion müssen in JEDEM Durchlauf zurückgesetzt werden!
        feed_min = 1.0
        ...
        # --- INNER LOOP: Die exakte Bisektion (Findet den physikalischen Flow) ---
        for bisection_it in range(60): 
            ...
            for i in range(anzahl_membranen):
                ...
                # --- DAS SCHEIBEN-MODELL (Ohne künstliche Caps, rein physikalisch) ---
                for j in range(n_seg):
                    q_p_seg = flow_local * 0.1 # Initiale Schätzung für dieses Segment
                    
                    for _ in range(8): # Schnelle Konvergenz für das Einzelsegment
                        ...
                        # Soft-Limit: Eine Scheibe kann maximal 95% ihres lokalen Wassers saugen
            ...
            # --- PHYSIK-CHECK: Wieviel Konzentrat LÄSST die Drossel durch? ---
            ...
            if q_c_total_calc > q_c_throttle:
                feed_max = q_feed_guess # Zu viel Konzentrat generiert -> Pumpe muss drosseln (Flow runter)
            else:
                feed_min = q_feed_guess # Drossel lässt mehr durch als vorhanden -> Pumpe zieht an (Flow rauf)

            # Bisektions-Abbruch, wenn der Feed-Flow auf 0,5 l/h genau bestimmt ist
            if abs(feed_max - feed_min) < 0.5:
                break

        # --- UPDATE FÜR DIE NÄCHSTE ÄUSSERE ITERATION ---
