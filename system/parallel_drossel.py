...
def simuliere_parallel_drossel(hydraulik, drossel_vorgabe_mm, m_flaeche, m_test_flow,
    ...
    # Outer Loop: Justiert die Gleichverteilung in T-Stücken
    for outer_it in range(5):
        total_permeat_guess = 200.0
        
        # Inner Loop: Newton-Bisektion auf Basis des Ventil-Rückstaus ("Variante C")
        for bisection_it in range(60): 
            ...
            for i in range(anzahl_membranen):
                ...
                # Exakte Vorwärtsintegration ohne künstliche Caps
            ...
            # Abgleich: Wieviel Wasser lässt das Ventil bei diesem Druck durch?
            v_theo = math.sqrt(2 * (p_vor_ventil * 100000.0) / rho)
            q_c_throttle = c_d * area_drossel_m2 * v_theo * 3600.0 * 1000.0

            # Physikalischer Check: Staut es sich an der Drossel?
            if q_c_total_calc > q_c_throttle:
                q_max = q_feed_guess  # Pumpe muss gedrosselt werden
            else:
                q_min = q_feed_guess  # Pumpe hat noch Reserven
