...
with st.sidebar:
    ...
    with st.expander("3. Rohrleitungen Zuleitung", expanded=False):
        ...
        # --- PARALLELE AUFTEILUNG (T-STÜCK LOGIK) ---
        if schaltung == "Parallel (Aufteilung)":
            ...
            if hat_t_stueck:
                ...
                d_a = ca1.number_input("Ø Strang A", value=15.0)
                l_a = ca2.number_input("Länge A", value=1000.0)
                b_a = ca3.number_input("Bögen A", 0, 10, 0)
                
                sub_a = st.checkbox("↳ Strang A in A1 & A2 aufteilen")
                if sub_a:
                    c_sub_a1, c_sub_a2, c_sub_a3 = st.columns(3)
                    d_a1 = c_sub_a1.number_input("Ø A1", value=10.0)
                    l_a1 = c_sub_a2.number_input("Länge A1", value=500.0)
                    b_a1 = c_sub_a3.number_input("Bögen A1", 0, 10, 0)
                    
                    d_a2 = c_sub_a1.number_input("Ø A2", value=10.0)
                    l_a2 = c_sub_a2.number_input("Länge A2", value=500.0)
                    b_a2 = c_sub_a3.number_input("Bögen A2", 0, 10, 0)
                ...
                d_b = cb1.number_input("Ø Strang B", value=15.0)
                l_b = cb2.number_input("Länge B", value=1000.0)
                b_b = cb3.number_input("Bögen B", 0, 10, 0)
                
                sub_b = st.checkbox("↳ Strang B in B1 & B2 aufteilen")
                if sub_b:
                    c_sub_b1, c_sub_b2, c_sub_b3 = st.columns(3)
                    d_b1 = c_sub_b1.number_input("Ø B1", value=10.0)
                    l_b1 = c_sub_b2.number_input("Länge B1", value=500.0)
                    b_b1 = c_sub_b3.number_input("Bögen B1", 0, 10, 0)
                    
                    d_b2 = c_sub_b1.number_input("Ø B2", value=10.0)
                    l_b2 = c_sub_b2.number_input("Länge B2", value=500.0)
                    b_b2 = c_sub_b3.number_input("Bögen B2", 0, 10, 0)
                ...
                # --- LIVE BALANCING AUSGABE (Nur im UI sichtbar) ---
                st.divider()
                if (r_a_tot + r_b_tot) > 0:
                    pct_a = math.sqrt(r_b_tot) / (math.sqrt(r_a_tot) + math.sqrt(r_b_tot))
                else: pct_a = 0.5
                pct_b = 1.0 - pct_a
                
                st.success(f"**Live Balancing im Netzwerk:**\nStrang A führt {pct_a*100:.1f}% | Strang B führt {pct_b*100:.1f}%")
                
                if sub_a and (r_a1 + r_a2) > 0: 
                    pct_a1 = math.sqrt(r_a2) / (math.sqrt(r_a1) + math.sqrt(r_a2))
                    st.info(f"↳ Aufteilung A: A1 ({pct_a1*100:.1f}%) | A2 ({(1-pct_a1)*100:.1f}%)")
                if sub_b and (r_b1 + r_b2) > 0: 
                    pct_b1 = math.sqrt(r_b2) / (math.sqrt(r_b1) + math.sqrt(r_b2))
                    st.info(f"↳ Aufteilung B: B1 ({pct_b1*100:.1f}%) | B2 ({(1-pct_b1)*100:.1f}%)")
