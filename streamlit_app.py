...
# --- Hydraulik-Kernfunktionen (Physikalisch exakte Widerstandsmodelle) ---
def berechne_hydraulischen_widerstand(d_inner_mm, laenge_mm, drosseln_liste, anzahl_90_grad):
    """Berechnet den strömungsmechanischen Widerstand R. (P = R * Q^2)"""
    if d_inner_mm <= 0: return 1e12 # Unendlicher Widerstand (Verhinderung von Div/0)
    d_m = d_inner_mm / 1000
    ...
    # R = Zeta * (Dichte/2) / Fläche^2 (Dichte Wasser = 1000, 1000/2 = 500)
    r_wert = zeta_total * 500 / (area**2)
    return r_wert

def r_parallel(r1, r2):
    """Berechnet den Gesamtwiderstand zweier paralleler Stränge"""
    if r1 <= 0: return r2
    if r2 <= 0: return r1
    return (1/math.sqrt(r1) + 1/math.sqrt(r2))**-2

def empfehle_drossel_durchmesser(flow_lh, delta_p_bar):
    """Berechnet den Blendendurchmesser für das End-Regelventil"""
...
with st.sidebar:
    ...
    st.header("3. Zuleitung ZUR Pumpe (Saugseite)")
    with st.expander("Saug-Schlauch definieren"):
    ...
    st.header("4. Zuleitung NACH Pumpe (Druckseite)")
    ...
    # Aufteilung NUR bei Parallelschaltung möglich!
    if schaltung == "Parallel":
        ...
        if hat_t_stueck:
            ...
            # Gesamtwiderstand des Split-Netzwerks
...
# 2. Druckverluste mit realem Fluss in Pa umrechnen (P = R * Q^2)
q_ms = (q_feed_start_lh / 1000) / 3600
p_verlust_saug = (r_saug * q_ms**2) / 100000 # Umrechnung in bar
p_verlust_druck_haupt = (r_druck_haupt * q_ms**2) / 100000
p_verlust_netzwerk = (r_netzwerk * q_ms**2) / 100000 if (schaltung == "Parallel" and hat_t_stueck) else 0

# 3. Effektiver Startdruck für Membranen
...
for i in range(anzahl_membranen):
    ...
    if schaltung == "In Reihe (Konzentrat -> Feed)":
        if i < anzahl_membranen - 1:
            ...
            current_p = p_in - 0.2 - p_verlust_zwischen # 0.2 bar Spacer-Verlust der vorherigen Membran
...
with h1:
    ...
    if p_verlust_saug > 0.3: st.warning("⚠️ Kavitationsgefahr! Saugwiderstand zu hoch. Vergrößere den Schlauchdurchmesser oder entferne Drosseln.")
with h2:
    st.markdown("**Druckseite (Pumpe -> Membran)**")
    st.write(f"- Verlust Hauptleitung: {p_verlust_druck_haupt:.3f} bar")
    if schaltung == "Parallel" and hat_t_stueck:
...
v3.metric("Empfohlene Blenden-Bohrung", f"Ø {empfohlene_drossel_mm:.2f} mm")
